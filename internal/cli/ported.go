...
func newInfoCmd() *cobra.Command {
	return &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			fmt.Println()
			fmt.Println("  Layers:")
			fmt.Println("    0  File & Session     init, config, auth, exec, whoami, open, status, new-file")
			fmt.Println("    1  Primitives         node (+ boolean, svg, variant-set), style, text, layout")
			fmt.Println("    2  Patterns           card (8), ui (29), fx (14), image")
			fmt.Println("    3  Deliverables       make (37 templates + changelog)")
